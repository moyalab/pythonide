const e=[{id:"setup",title:"환경설정",description:"USB 동글 드라이버 설치 안내",icon:"settings",setup:{intro:"카미봇과 PC를 시리얼 포트로 연결하려면 USB 동글 드라이버를 먼저 설치해야 합니다. 아래 절차를 따라 드라이버 설치를 진행해 주세요.",steps:['아래 "드라이버 다운로드" 버튼을 눌러 설치 파일(CDM21228_Setup.zip)을 내려받습니다.','내려받은 zip 파일을 마우스 오른쪽 버튼으로 클릭하고 "압축 풀기"를 선택해 압축을 해제합니다.',"압축 해제된 폴더 안의 CDM21228_Setup.exe 파일을 실행해 안내에 따라 드라이버를 설치합니다.","설치가 끝나면 USB 동글을 PC에 꽂고 카미봇 본체의 전원을 켜 연결을 확인합니다."],note:"설치 후에도 포트가 인식되지 않으면 PC를 재부팅하거나 다른 USB 포트에 연결해 주세요.",download:{href:"/drivers/CDM21228_Setup.zip",filename:"CDM21228_Setup.zip",label:"드라이버 다운로드 (CDM21228_Setup.zip)"},pairing:{title:"동글과 로봇 연결하기",intro:"드라이버 설치가 끝나면 카미봇 본체와 USB 동글(카미봇 동글)을 다음 절차로 페어링합니다.",steps:["카미봇 본체와 카미봇 동글을 준비합니다.","카미봇 본체의 전원을 켭니다. 본체의 LED가 켜지고 여러 색상으로 바뀝니다.","카미봇 동글을 PC의 USB 포트에 꽂고, 카미봇 본체를 동글에 최대한 가까이 가져갑니다.","카미봇 동글의 버튼을 누릅니다.","카미봇 본체의 LED가 파란색으로 바뀌면 하드웨어 연결이 완료된 상태입니다."]}}},{id:"getting-started",title:"시작하기",description:"연결, 정지, 대기 등 기본 명령",icon:"play_circle",entries:[{name:"KamibotPi(port)",summary:"카미봇과 시리얼 포트로 연결합니다.",details:`Args:
...
bot = KamibotPi()`},{name:"bot.close()",summary:"연결을 종료하고 프로그램을 끝냅니다.",details:`시리얼 포트를 닫은 뒤 sys.exit(0)을 호출합니다.
Returns: None`,example:"bot.close()"},{name:"bot.disconnect()",summary:"연결만 끊고 프로그램은 계속 실행합니다.",details:`시리얼 포트를 닫지만 프로그램을 종료하지는 않습니다.
Returns: None`,example:"bot.disconnect()"},{name:"bot.init()",summary:"로봇을 초기 상태로 되돌립니다.",details:`내부 상태를 리셋합니다.
Returns: None`,example:"bot.init()"},{name:"bot.delay(sec)",summary:"주어진 시간(초)만큼 기다립니다.",details:`Args:
  sec (float): 초
Returns: None`,example:"bot.delay(1.5)"},{name:"bot.delayms(ms)",summary:"주어진 시간(밀리초)만큼 기다립니다.",details:`Args:
  ms (int): 밀리초
Returns: None`,example:"bot.delayms(500)"},{name:"bot.wait(ms)",summary:"delayms와 동일하게 밀리초 단위로 기다립니다.",details:`Args:
  ms (int): 밀리초
Returns: None`,example:"bot.wait(200)"},{name:"bot.stop()",summary:"이동 중인 로봇을 멈춥니다.",details:`Args: 없음
Returns: None`,example:"bot.stop()"}]},{id:"sound",title:"소리",description:"비프음과 음계 연주",icon:"music_note",entries:[{name:"bot.beep(sec=0.2)",summary:'"삐" 소리를 냅니다. 재생 시간을 초 단위로 지정할 수 있습니다.',details:`Args:
  sec (float): 재생 시간(초). 생략 시 0.2초. 범위 0.1 ~ 25.5초
Returns: None

`,example:{description:"beep() 함수는 60번 음계(C4, 중간 도)를 sec 초 동안 재생합니다.",code:`bot.beep()       # 0.2초
bot.beep(1)      # 1초
bot.beep(0.5)    # 0.5초`}},{name:"bot.melody(scale, sec)",summary:"지정한 음계를 주어진 시간만큼 재생합니다.",details:`Args:
  scale (int): 음계 (0 ~ 83). Note.C4 같은 상수도 사용 가능
  sec (int): 재생 시간(초)
Returns: None

`,example:`from pibot import KamibotPi, Note

bot = KamibotPi()
bot.melody(Note.C4, 1)
bot.melody(Note.E4, 1)
bot.melody(Note.G4, 1)`,warning2:`Note 상수를 쓰려면 from pibot import Note 가 필요합니다.
...
[12] bot.close()
   시리얼 포트를 닫고 종료합니다.`,code:`# Import the robot driver and the Note table.
...
※ 비교 포인트: 경찰차는 T=0.3초로 빠르게, 앰뷸런스는 T=0.9초로 느리게. 같은 "두 음 교대" 패턴이라도 시간만 바꾸면 전혀 다른 사이렌이 됩니다.`,code:`# Import the robot driver and the Note table.
from pibot import KamibotPi, Note
...
[39] bot.close()
   시리얼 포트를 닫고 프로그램을 종료합니다.`,code:`# Import the robot driver and the Note table.
...
try:
    ...
    bot.turn_led(*OFF)
    bot.close()`}]},{id:"speed-control",title:"속도 제어",description:"바퀴 속도를 직접 지정해 이동",icon:"speed",entries:[{name:"bot.go_forward_speed(lspeed, rspeed)",summary:"두 바퀴 속도를 지정해 앞으로 이동합니다.",details:`Args:
  lspeed (int): 왼쪽 바퀴 속도
  rspeed (int): 오른쪽 바퀴 속도
Returns: None`,example:"bot.go_forward_speed(100, 100)"},{name:"bot.go_backward_speed(lspeed, rspeed)",summary:"두 바퀴 속도를 지정해 뒤로 이동합니다.",details:`Args:
  lspeed (int): 왼쪽 바퀴 속도
  rspeed (int): 오른쪽 바퀴 속도
Returns: None`,example:"bot.go_backward_speed(80, 80)"},{name:"bot.go_left_speed(speed)",summary:"왼쪽으로 회전합니다 (오른쪽 바퀴만 회전).",details:`Args:
  speed (int): 회전 속도
Returns: None`,example:"bot.go_left_speed(100)"},{name:"bot.go_right_speed(speed)",summary:"오른쪽으로 회전합니다 (왼쪽 바퀴만 회전).",details:`Args:
  speed (int): 회전 속도
Returns: None`,example:"bot.go_right_speed(100)"},{name:"bot.go_dir_speed(ldir, lspeed, rdir, rspeed)",summary:"두 바퀴의 방향과 속도를 각각 지정합니다.",details:`Args:
  ldir (str): "f" 앞으로, "b" 뒤로
  lspeed (int): 왼쪽 속도
  rdir (str): "f" 앞으로, "b" 뒤로
  rspeed (int): 오른쪽 속도
Returns: None`,example:'bot.go_dir_speed("f", 100, "b", 100)'}],entriesAfter:[{name:"예제: 앞으로 2초 가기",summary:"두 바퀴를 같은 속도로 굴려 2초 동안 직진한 뒤 멈춥니다.",details:`go_forward_speed는 모터를 "켜기만" 합니다. stop()을 부르기 전까지 계속 굴러가므로, 원하는 시간만큼 delay로 기다린 다음 멈춰야 합니다.
...
bot.stop()
bot.close()`}]},{id:"precision",title:"정밀 제어 (cm/초/스텝)",description:"단위를 지정해 정확하게 이동",icon:"straighten",entries:[{name:"bot.move_forward_unit(value, opt, speed)",summary:"앞으로 지정 단위만큼 이동합니다.",details:`Args:
  value (int): 이동값
  opt (str): "-l" cm, "-t" 초, "-s" 스텝
  speed (int): 속도
Returns: None`,example:'bot.move_forward_unit(20, "-l", 50)'},{name:"bot.move_backward_unit(value, opt, speed)",summary:"뒤로 지정 단위만큼 이동합니다.",details:`Args:
  value (int): 이동값
  opt (str): "-l" cm, "-t" 초, "-s" 스텝
  speed (int): 속도
Returns: None`,example:'bot.move_backward_unit(20, "-l", 50)'},{name:"bot.move_left_unit(value, opt, speed)",summary:"왼쪽으로 지정 단위만큼 이동합니다.",details:`Args:
  value (int): 이동값
  opt (str): "-l" cm, "-t" 초, "-s" 스텝
  speed (int): 속도
Returns: None`,example:'bot.move_left_unit(10, "-l", 50)'},{name:"bot.move_right_unit(value, opt, speed)",summary:"오른쪽으로 지정 단위만큼 이동합니다.",details:`Args:
  value (int): 이동값
  opt (str): "-l" cm, "-t" 초, "-s" 스텝
  speed (int): 속도
Returns: None`,example:'bot.move_right_unit(10, "-l", 50)'},{name:"bot.turn_left_speed(value, speed)",summary:"제자리에서 왼쪽으로 회전합니다.",details:`Args:
  value (int): 회전각도
  speed (int): 속도
Returns: None`,example:"bot.turn_left_speed(90, 50)"},{name:"bot.turn_right_speed(value, speed)",summary:"제자리에서 오른쪽으로 회전합니다.",details:`Args:
  value (int): 회전각도
  speed (int): 속도
Returns: None`,example:"bot.turn_right_speed(90, 50)"},{name:"bot.move_step(ldir, lstep, rdir, rstep)",summary:"두 모터의 방향과 스텝수를 각각 지정합니다.",details:`Args:
  ldir, rdir (str): "f" 앞으로 / "b" 뒤로
  lstep, rstep (int): 스텝수
Returns: None`,example:'bot.move_step("f", 200, "f", 200)'},{name:"bot.move_time(ldir, lsec, rdir, rsec)",summary:"두 모터의 방향과 작동 시간을 각각 지정합니다.",details:`Args:
  ldir, rdir (str): "f" 앞으로 / "b" 뒤로
  lsec, rsec (int): 작동 시간(초)
Returns: None`,example:'bot.move_time("f", 2, "f", 2)'},{name:"bot.turn_continous(dir, speed)",summary:"지정 방향으로 계속 회전합니다.",details:`Args:
  dir (str): "l" 왼쪽 / "r" 오른쪽
  speed (int): 속도
Returns: None`,example:'bot.turn_continous("r", 100)'}],entriesAfter:[{name:"예제 1: 30cm 갔다가 돌아오기",summary:'"-l" 옵션으로 정확히 30cm를 직진하고, 180°로 돌아 다시 출발점으로 돌아옵니다. 단위·각도의 기본을 익히는 첫 예제입니다.',details:`학습 포인트:
...
bot.close()`}]},{id:"top-motor",title:"탑 모터",description:"상단 스텝 모터(펜대 등) 제어",icon:"rotate_right",entries:[{name:"bot.top_motor_degree(dir, value, speed)",summary:"탑 모터를 지정 각도만큼 회전시킵니다.",details:`Args:
  dir (str): "l" 왼쪽 / "r" 오른쪽
  value (int): 각도
  speed (int): 속도
Returns: None`,example:'bot.top_motor_degree("r", 90, 50)'},{name:"bot.top_motor_abspos(degree, speed)",summary:"탑 모터를 절대 각도 위치로 이동시킵니다.",details:`Args:
  degree (int): 절대 각도 (최대 65000)
  speed (int): 속도
Returns: None`,example:"bot.top_motor_abspos(180, 50)"},{name:"bot.top_motor_time(dir, value, speed)",summary:"탑 모터를 지정 시간만큼 회전시킵니다.",details:`Args:
  dir (str): "l" 왼쪽 / "r" 오른쪽
  value (int): 시간(초)
  speed (int): 속도
Returns: None`,example:'bot.top_motor_time("l", 3, 50)'},{name:"bot.top_motor_round(dir, value, speed)",summary:"탑 모터를 지정 회전수만큼 회전시킵니다.",details:`Args:
  dir (str): "l" 왼쪽 / "r" 오른쪽
  value (int): 회전수
  speed (int): 속도
Returns: None`,example:'bot.top_motor_round("r", 1, 50)'},{name:"bot.top_motor_stop()",summary:"탑 모터 회전을 정지시킵니다.",details:`Args: 없음
Returns: None`,example:"bot.top_motor_stop()"}],entriesAfter:[{name:"예제 1: 펜 90° 들어올렸다가 다시 내리기",summary:"오른쪽으로 90° 돌렸다가, 잠깐 쉰 뒤 왼쪽으로 90° 돌려서 정확히 처음 자리로 돌아옵니다. 탑 모터를 처음 다룰 때 가장 안전한 출발점입니다.",details:`학습 포인트:
...
bot.close()`}]},{id:"shapes",title:"도형 그리기",description:"삼각형, 사각형, 별, 원호 등 그리기",icon:"category",entries:[{name:"bot.draw_tri(len)",summary:"한 변의 길이를 지정해 삼각형을 그립니다.",details:`Args:
  len (int): 한 변의 길이(cm)
Returns: None`,example:"bot.draw_tri(20)"},{name:"bot.draw_rect(len)",summary:"한 변의 길이를 지정해 사각형을 그립니다.",details:`Args:
  len (int): 한 변의 길이(cm)
Returns: None`,example:"bot.draw_rect(20)"},{name:"bot.draw_penta(len)",summary:"한 변의 길이를 지정해 오각형을 그립니다.",details:`Args:
  len (int): 한 변의 길이(cm)
Returns: None`,example:"bot.draw_penta(20)"},{name:"bot.draw_hexa(len)",summary:"한 변의 길이를 지정해 육각형을 그립니다.",details:`Args:
  len (int): 한 변의 길이(cm)
Returns: None`,example:"bot.draw_hexa(20)"},{name:"bot.draw_star(len)",summary:"한 변의 길이를 지정해 별을 그립니다.",details:`Args:
  len (int): 한 변의 길이(cm)
Returns: None`,example:"bot.draw_star(20)"},{name:"bot.draw_circle(len)",summary:"반지름을 지정해 원을 그립니다.",details:`Args:
  len (int): 반지름
Returns: None`,example:"bot.draw_circle(15)"},{name:'bot.draw_semicircle(len, side="l")',summary:"반원을 그립니다.",details:`Args:
  len (int): 반지름
  side (str): "l" 왼쪽 / "r" 오른쪽
Returns: None`,example:'bot.draw_semicircle(15, "l")'},{name:"bot.draw_arc(radius, value, mode=0)",summary:"시간 또는 각도 단위로 원호를 그립니다.",details:`Args:
  radius (int): 반지름
  value (int): mode가 0이면 시간(초), 1이면 각도
  mode (int): 0 시간 / 1 각도
Returns: None`,example:"bot.draw_arc(15, 2, 0)"}],entriesAfter:[{name:"예제 1: 도형 4총사 (삼·사·오·육각형 줄지어 그리기)",summary:"draw_tri / draw_rect / draw_penta / draw_hexa를 차례로 호출해 변의 수가 다른 네 가지 도형을 한 줄에 늘어놓습니다.",details:`학습 포인트:
...
bot.close()`}]},{id:"sensors",title:"센서",description:"물체·라인·컬러 센서 값 읽기",icon:"sensors",entries:[{name:"bot.get_object_detect(opt=True)",summary:"좌·우 물체 감지 센서 값을 읽습니다.",details:`Args:
  opt (bool): True 동작 / False 멈춤
Returns:
  (left, right) — 좌·우 감지값`,example:`left, right = bot.get_object_detect()
print(left, right)`},{name:"bot.get_line_sensor(opt=True)",summary:"왼쪽·중앙·오른쪽 라인 센서 값을 읽습니다.",details:`Args:
  opt (bool): True 동작 / False 멈춤
Returns:
  (left, center, right) — 라인 감지값`,example:`l, c, r = bot.get_line_sensor()
print(l, c, r)`},{name:"bot.get_color_sensor(opt=True)",summary:"컬러 센서로 색상 인덱스를 읽습니다.",details:`Args:
  opt (bool): True 동작 / False 멈춤
Returns:
  color (int) — 색상 인덱스`,example:`color = bot.get_color_sensor()
print(color)`},{name:"bot.get_color_elements(opt=True)",summary:"컬러 센서로 RGB 값을 읽습니다.",details:`Args:
  opt (bool): True 동작 / False 멈춤
Returns:
  (r, g, b) — RGB 성분`,example:`r, g, b = bot.get_color_elements()
print(r, g, b)`}],entriesAfter:[{name:"예제 1: 물체를 만나면 멈추기 (장애물 감지)",summary:"카미봇이 천천히 직진하면서 앞쪽 물체 센서를 계속 확인합니다. 좌·우 어느 쪽이든 물체가 감지되면 즉시 멈추고 빨간 LED를 켭니다.",details:`학습 포인트 — "센서를 반복해서 확인하기":
...
while True:
    ...
    if left == 1 and center == 1 and right==1:
        ...
        robot.go_forward_speed(80, 30)  # 우회전`}]},{id:"misc",title:"라인트레이서 / 정보",description:"라인트레이서 토글, 배터리·버전 조회",icon:"route",entries:[{name:"bot.toggle_linetracer(mode, speed=100)",summary:"라인트레이서 기능을 켜거나 끕니다.",details:`Args:
  mode (bool): True 켜기 / False 끄기
  speed (int): 라인트레이서 속도
Returns: None`,example:"bot.toggle_linetracer(True, 80)"},{name:"bot.get_battery()",summary:"현재 배터리 잔량을 읽어옵니다.",details:`Args: 없음
Returns:
  battery (int) — 배터리값`,example:`level = bot.get_battery()
print(level)`},{name:"bot.get_version()",summary:"펌웨어 버전을 조회합니다.",details:`Args: 없음
Returns: None (출력은 시리얼 응답으로)`,example:"bot.get_version()"}]},{id:"basic-move",title:"기본 이동 (맵보드)",description:"맵보드 칸 단위로 이동/회전",icon:"arrow_forward",notice:"이 기능은 전용 맵보드가 필요합니다.",entries:[{name:'bot.move_forward(value, opt="-l")',summary:"앞으로 지정한 칸 수만큼 이동합니다.",details:`Args:
  value (int): 이동 칸수
  opt (str): "-l" 라인맵보드, "-b" 블록맵보드
Returns: None`,example:"bot.move_forward(2)"},{name:"bot.move_backward(value)",summary:"뒤로 지정한 칸 수만큼 이동합니다 (블록맵보드 전용).",details:`Args:
  value (int): 이동 칸수
Returns: None`,example:"bot.move_backward(1)"},{name:'bot.turn_left(value=1, opt="-l")',summary:"왼쪽으로 회전합니다.",details:`Args:
  value (int): 회전 횟수 (라인맵에서는 무시되고 1회만 실행)
  opt (str): "-l" 라인맵, "-b" 블록맵
Returns: None`,example:"bot.turn_left()"},{name:'bot.turn_right(value=1, opt="-l")',summary:"오른쪽으로 회전합니다.",details:`Args:
  value (int): 회전 횟수 (라인맵에서는 무시되고 1회만 실행)
  opt (str): "-l" 라인맵, "-b" 블록맵
Returns: None`,example:"bot.turn_right()"},{name:'bot.turn_back(value=1, opt="-l")',summary:"뒤로 (180도) 회전합니다.",details:`Args:
  value (int): 회전 횟수 (라인맵에서는 무시되고 1회만 실행)
  opt (str): "-l" 라인맵, "-b" 블록맵
